...
einkaeufe <- read_csv("data/supermarkt/einkaeufe.csv")
kundenprofile <- read_csv("data/supermarkt/kundenprofile.csv")
preise <- read_csv("data/supermarkt/preise.csv")
...
tabyl(einkaeufe, kunden_id)
tabyl(kundenprofile, kunden_id)
```
...
einkaeufe_kosten_sum <- einkaeufe_kosten %>% 
  group_by(kunden_id) %>% 
  summarise(totkosten = sum(kosten))
...
kunden_tab <- einkaeufe_kosten_sum %>% 
  ...
  select(ends_with("name"), totkosten, email)
